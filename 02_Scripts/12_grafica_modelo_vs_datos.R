...
### Casos Fallecidos del modelo ================================================
prueba3 <- beta_t_out %>% select(c(days),M1,M2,M3,M4)
prueba4 <- data.frame(grupos = rep(c("M1","M2","M3","M4"), each=398),
                      FECHA_DEF = rep(prueba3$days, 4),
                      muertes_totales=c(prueba3$M1,prueba3$M2,prueba3$M3,prueba3$M4))
### Datos reales y del modelo --------------------------------------------------
concatenated_tibble_2 <- bind_rows(muertes_x_grupos_corte,prueba4)
### Ajuste de los datos ----- --------------------------------------------------
data_frame_filtrado2 <- concatenated_tibble_2 %>%
  filter(FECHA_DEF <= as.Date("2021-04-07"))
data_frame2 <- data_frame_filtrado2 %>%
  mutate(tipo = ifelse(grupos %in% c("M1", "M2", "M3", "M4"), "Modelo", "Casos"))
muertes_solo_filt <- filter(data_frame2, tipo == "Casos")
modelo_m_solo_filt <- filter(data_frame2, tipo == "Modelo")
### Grafica --------------------------------------------------------------------
